...
void generate_image(SIM* sim, float* generated_image, float temperature, unsigned char* d_input_tokens) {
    ...
    printf("Generating image pixel by pixel...\n");
    ...
    for (int pixel = 0; pixel < image_pixels - 1; pixel++) {
        ...
        CHECK_CUDA(cudaMemcpy(d_input_tokens, h_tokens, image_pixels * sizeof(unsigned char), cudaMemcpyHostToDevice));
...
int main(int argc, char* argv[]) {
    ...
    for (int epoch = 0; epoch < num_epochs + 1; epoch++) {
        ...
        for (int batch = 0; batch < num_batches; batch++) {
            ...
            if (batch > 0 && batch % 500 == 0) {
                ...
                generate_image(sim, generated_image, 0.8f, d_input_tokens);
                
                // Save generated image
                char gen_filename[256];
                snprintf(gen_filename, sizeof(gen_filename), "generated_images/generated_epoch_%d_batch_%d.png", epoch, batch);
        ...
        if (epoch < num_epochs) {
            printf("Generating end-of-epoch sample...\n");
            float* generated_image = (float*)malloc(seq_len * sizeof(float));
            generate_image(sim, generated_image, 0.7f, d_input_tokens);
            
            char gen_filename[256];
            snprintf(gen_filename, sizeof(gen_filename), "generated_images/epoch_%d_final.png", epoch);
            save_mnist_image_png(generated_image, gen_filename);
            printf("Saved epoch %d final image: %s\n", epoch, gen_filename);
    ...
    for (int i = 0; i < 5; i++) {
        float* generated_image = (float*)malloc(seq_len * sizeof(float));
        generate_image(sim, generated_image, 0.6f + i * 0.1f, d_input_tokens);
        
        char gen_filename[256];
        snprintf(gen_filename, sizeof(gen_filename), "generated_images/final_sample_%d_temp_%.1f.png", i, 0.6f + i * 0.1f);
        save_mnist_image_png(generated_image, gen_filename);
        printf("Saved final sample %d: %s\n", i, gen_filename);
