...
__global__ static void generate_noise_kernel(float* noise, curandState* states, int total_elements) {
    ...
    if (idx < total_elements) {
        noise[idx] = curand_normal(&states[idx % 65536]); // Limit state array size
    }
}

// Add noise to clean patches (forward diffusion)
__global__ static void add_noise_kernel(float* noisy_patches, float* clean_patches, float* noise,
                                       float sqrt_alpha_bar, float sqrt_one_minus_alpha_bar, int total_elements) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < total_elements) {
...
__global__ static void add_position_encoding_kernel(float* embedded, int batch_size, int seq_len, int d_model) {
    int b = blockIdx.x;
    int t = blockIdx.y;
    int d = threadIdx.x;
    ...
    embedded[idx] += pos_encoding * 0.1f; // Scale down position encoding
}
...
SIM* init_sim(int d_model, int hidden_dim, int num_layers, int batch_size, cublasLtHandle_t cublaslt_handle) {
    ...
    // Initialize time embeddings with sinusoidal encoding
    float* h_time_embedding = (float*)malloc(time_emb_size * sizeof(float));
    for (int t = 0; t < MAX_TIMESTEPS; t++) {
        for (int d = 0; d < d_model; d++) {
            if (d % 2 == 0) {
                h_time_embedding[t * d_model + d] = sinf(t / powf(10000.0f, (2.0f * (d / 2)) / d_model));
            } else {
                h_time_embedding[t * d_model + d] = cosf(t / powf(10000.0f, (2.0f * ((d - 1) / 2)) / d_model));
            }
        }
    }
    ...
    CHECK_CUDA(cudaMalloc(&sim->d_clean_patches, patch_buffer_size * sizeof(float)));
    ...
    CHECK_CUDA(cudaMalloc(&sim->d_curand_states, 65536 * sizeof(curandState)));
    
    // Initialize curand states
    init_curand_states<<<256, 256>>>(sim->d_curand_states, time(NULL), 65536);
    CHECK_CUDA(cudaDeviceSynchronize());
    ...
    for (int t = 0; t < MAX_TIMESTEPS; t++) {
        float beta = MIN_BETA + (MAX_BETA - MIN_BETA) * t / (MAX_TIMESTEPS - 1);
        alpha_bar *= (1.0f - beta);
        sim->h_alpha_bars[t] = alpha_bar;
    }
...
void free_sim(SIM* sim) {
    ...
    cudaFree(sim->d_time_embedding);
    cudaFree(sim->d_time_embedding_grad);
    cudaFree(sim->d_time_embedding_m);
    cudaFree(sim->d_time_embedding_v);
    cudaFree(sim->d_clean_patches);
    cudaFree(sim->d_noisy_patches);
    cudaFree(sim->d_noise);
    cudaFree(sim->d_patch_embeds);
    cudaFree(sim->d_loss_result);
    cudaFree(sim->d_curand_states);
    
    free(sim->h_alpha_bars);
    free(sim);
}
...
void forward_pass_sim(SIM* sim, float* d_images, int timestep) {
    ...
    image_to_patches_kernel<<<grid_patches, block_patches>>>(sim->d_clean_patches, d_images, sim->batch_size);
    ...
    add_noise_kernel<<<num_blocks, block_size>>>(sim->d_noisy_patches, sim->d_clean_patches, sim->d_noise,
                                                 sqrt_alpha_bar, sqrt_one_minus_alpha_bar, patch_buffer_size);
    
    // Project noisy patches to d_model
    ...
    dim3 block_pos(sim->d_model);
    add_position_encoding_kernel<<<grid_pos, block_pos>>>(sim->d_patch_embeds, sim->batch_size, NUM_PATCHES, sim->d_model);
    ...
    // Project back to patch dimension (noise prediction)
    forward_pass_mlp(sim->output_mlp, sim->transformer->mlp_layers[sim->num_layers-1]->d_layer_output);
}
...
void backward_pass_sim(SIM* sim, int timestep) {
    ...
    // Accumulate time embedding gradients
...
void update_weights_sim(SIM* sim, float learning_rate) {
    ...
    adamw_update_kernel<<<time_blocks, block_size>>>(
...
void save_sim(SIM* sim, const char* filename) {
    ...
    char transformer_filename[256], input_mlp_filename[256], output_mlp_filename[256];
...
SIM* load_sim(const char* filename, int custom_batch_size, cublasLtHandle_t cublaslt_handle) {
    ...
    int d_model, stored_batch_size, hidden_dim, num_layers, t;
